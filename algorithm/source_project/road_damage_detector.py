...
async def run_detect_task(task_id: str) -> None:
    ...
    try:
        ...
        # 拿到当前事件循环，供同步回调里投递异步任务
        loop = asyncio.get_running_loop()

        def progress_callback(payload: dict) -> None:
            """
            注意：RoadDamageDetector.analyze_video() 是同步函数，
            它直接调用 progress_callback(payload)，不会 await。
            所以这里必须是普通 def，不能是 async def。
            """
            ...
            print(
                ...
                f"progress={progress}, current_frame={current_frame}, total_frames={total_frames}, "
                f"frame_bytes_len={len(frame_bytes) if frame_bytes else 0}, "
            ...
            # 关键：把进度信息真正写回任务状态，供 /api/task/{task_id} 返回
            task_manager.update_task(
                task_id,
                {
                    "status": "processing",
            ...
            # 同步回调里不能 await，只能把协程投递到当前事件循环
            if frame_bytes:
                asyncio.run_coroutine_threadsafe(
                    stream_manager.update_frame(task_id, frame_bytes),
                    loop,
                )
        ...
        task_manager.update_task(
            task_id,
            {
                ...
                # 尽量保留最终帧信息，别在完成时清零
                "current_frame": result.get("total_frames", 0),
